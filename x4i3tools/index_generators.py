...
def process_file_package(work_package):
    ...
    return (thr_coupledReactionEntries, thr_monitoredReactionEntries,
            thr_reactionCount, thr_sql_transactions, thr_buggyEntries)
...
def buildMainIndex():
    ...
    # build up the table
    if x4t.verbose:
        print(os.path.join(x4t.currentDBPath, '*', '*.x4'))

    
    files_to_process = glob.glob(os.path.join(x4t.currentDBPath, '*', '*.x4'))
    total_files = len(files_to_process)
    chunksize = min(total_files // x4t.nthreads, 50)

    assert len(files_to_process) > 0, 'No files found in ' + x4t.currentDBPath 

    if x4t.verbose:
        print('Will process {0} files in chunks of {1} using {2} threads.'.format(
            len(files_to_process), chunksize, x4t.nthreads
        ))
    ...
    for wp in tqdm(processed):
        (
            wp_coupledReactionEntries, wp_monitoredReactionEntries,
            wp_reactionCount, wp_sql_transactions, wp_buggyEntries
        ) = wp
    ...
    print('\tNumber of entries with reaction monitors sets:', len(monitoredReactionEntries))
    ...
    cursor.executemany("insert into theworks values(?,?,?,?,?,?,?,?,?,?)", sql_transactions)
